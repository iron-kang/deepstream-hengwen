...
gst_nvinfer_class_init (GstNvInferClass * klass)
{
  ...
  g_object_class_install_property (gobject_class, PROP_CONFIG_RELOAD,
      g_param_spec_string ("config-file-reload", "Config File Reload",
          "Reload the configuration file for this instance of nvinfer",
          DEFAULT_CONFIG_FILE_PATH,
          (GParamFlags) (G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS |
              GST_PARAM_MUTABLE_PLAYING)));
    ...
    const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_MODEL_ENGINEFILE:
      ...
      break;
    case PROP_CONFIG_RELOAD:
      {
        g_print("%s: reload config.....\n", __func__);
        LockGMutex lock (nvinfer->process_lock);
        const std::string cfg_path (g_value_get_string (value));
        impl->triggerNewModel (cfg_path, MODEL_LOAD_FROM_CONFIG);
        g_free (nvinfer->config_file_path);
...
GST_PLUGIN_DEFINE (GST_VERSION_MAJOR, GST_VERSION_MINOR, nvdsgst_infer,
    DESCRIPTION, nvinfer_plugin_init, "5.1", LICENSE, BINARY_PACKAGE, URL)
